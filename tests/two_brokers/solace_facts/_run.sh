...
# Run
  ...
  playbook=$(assertFile $scriptLogName "$scriptDir/exceptions.bad-solace-cloud-config.playbook.yml") || exit
  ansible-playbook \
                    --forks 1 \
                    -i $solaceCloudInventory \
                    $playbook \
                    --extra-vars "SOLACE_CLOUD_API_TOKEN=$SOLACE_CLOUD_API_TOKEN_RESTRICTED_PERMISSIONS"
  code=$?; if [[ $code != 0 ]]; then echo ">>> ERROR - $code - script:$scriptLogName, playbook:$playbook"; exit 1; fi

  playbooks=(
    "$scriptDir/main.playbook.yml"
    "$scriptDir/examples.playbook.yml"
    "$scriptDir/exceptions.1.playbook.yml"
    "$scriptDir/exceptions.2.playbook.yml"
  )
