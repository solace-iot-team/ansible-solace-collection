...
short_description: jndi topic
description:
- "Configure a JNDI Topic object on a Message Vpn. Allows addition, removal and configuration of JNDI Topic objects in an idempotent manner."
notes:
...
options:
  name:
    description: Name of the JNDI topic. Maps to 'topicName' in the API.
    required: true
    type: str
    aliases: [topic]
...
class SolaceJNDITopicTask(SolaceBrokerCRUDTask):
    ...
    def get_func(self, vpn_name, jndi_topic_name):
    ...
    def create_func(self, vpn_name, jndi_topic_name, settings=None):
        ...
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG,
                      'msgVpns', vpn_name, 'jndiTopics']
        return self.sempv2_api.make_post_request(self.get_config(), path_array, data)

    def update_func(self, vpn_name, jndi_topic_name, settings=None, delta_settings=None):
