...
short_description: cancel replay on queue
description:
- "Cancel replay on a Queue object."
notes:
- "Module Sempv2 Action: https://docs.solace.com/API-Developer-Online-Ref-Documentation/swagger-ui/action/index.html#/queue/doMsgVpnQueueCancelReplay"
options:
  queue_name:
...
class SolaceQueueCancelReplayTask(SolaceBrokerActionTask):
    ...
    def put_func(self, vpn_name, queue_name, settings=None):
        # PUT /msgVpns/{msgVpnName}/queues/{queueName}/cancelReplay
        data = {
        }
...
def run_module():
    ...
    solace_task = SolaceQueueCancelReplayTask(module)
    solace_task.execute()
