...
short_description: jndi queue
description:
- "Configure a JNDI Queue object on a Message Vpn. Allows addition, removal and configuration of JNDI Queue objects in an idempotent manner."
notes:
...
options:
  name:
    description: Name of the JNDI queue. Maps to 'queueName' in the API.
    required: true
    type: str
...
class SolaceJNDIQueueTask(SolaceBrokerCRUDTask):
    ...
    def get_func(self, vpn_name, jndi_queue_name):
    ...
    def create_func(self, vpn_name, jndi_queue_name, settings=None):
        ...
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG,
                      'msgVpns', vpn_name, 'jndiQueues']
        return self.sempv2_api.make_post_request(self.get_config(), path_array, data)

    def update_func(self, vpn_name, jndi_queue_name, settings=None, delta_settings=None):
...
def run_module():
    ...
    solace_task = SolaceJNDIQueueTask(module)
    solace_task.execute()
