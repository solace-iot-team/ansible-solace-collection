...
short_description: list of publish topic exceptions on an acl profile
description:
...
options:
  names:
    description: The publish topic. Maps to 'publishTopicException' in the SEMP v2 API.
    required: true
    type: list
    aliases: [topics]
    elements: str
  acl_profile_name:
    ...
    type: str
  topic_syntax:
    description: The topic syntax. Maps to 'publishTopicExceptionSyntax' in the SEMP v2 API.
    required: false
    default: "smf"
    type: str
    choices:
...
class SolaceACLPublishTopicExceptionsTask(SolaceBrokerCRUDListTask):
    ...
    def create_func(self, vpn_name, acl_profile_name, topic_syntax, topic, settings=None):
        ...
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG,
                      'msgVpns', vpn_name,
                      'aclProfiles', acl_profile_name, 'publishTopicExceptions']
...
def run_module():
    ...
    solace_task = SolaceACLPublishTopicExceptionsTask(module)
    solace_task.execute()
