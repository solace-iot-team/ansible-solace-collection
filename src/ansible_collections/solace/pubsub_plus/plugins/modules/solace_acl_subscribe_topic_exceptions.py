...
short_description: list of subscribe topic exceptions on an acl profile
description:
...
notes:
- "Module Sempv2 Config: https://docs.solace.com/API-Developer-Online-Ref-Documentation/swagger-ui/config/index.html#/aclProfile/createMsgVpnAclProfileSubscribeTopicException"
options:
  names:
    description: The subscribe topic. Maps to 'subscribeTopicException' in the SEMP v2 API.
    required: true
    type: list
    aliases: [topics]
    elements: str
  acl_profile_name:
    ...
    type: str
  topic_syntax:
    description: The topic syntax. Maps to 'subscribeTopicExceptionSyntax' in the SEMP v2 API.
    required: false
    default: "smf"
    type: str
    choices:
...
class SolaceACLSubscribeTopicExceptionsTask(SolaceBrokerCRUDTopicExListTask):
    ...
    def create_func(self, vpn_name, acl_profile_name, topic_syntax, topic, settings=None):
        ...
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG,
                      'msgVpns', vpn_name,
                      'aclProfiles', acl_profile_name, 'subscribeTopicExceptions']
        return self.sempv2_api.make_post_request(self.get_config(), path_array, data)

    def delete_func(self, vpn_name, acl_profile_name, topic_syntax, topic):
        # DELETE /msgVpns/{msgVpnName}/aclProfiles/{aclProfileName}/subscribeTopicExceptions/{subscribeTopicExceptionSyntax},{subscribeTopicException}
        ex_uri = ','.join([topic_syntax, topic])
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG,
                      'msgVpns', vpn_name,
                      'aclProfiles', acl_profile_name,
                      'subscribeTopicExceptions', ex_uri]
...
def run_module():
    ...
    solace_task = SolaceACLSubscribeTopicExceptionsTask(module)
    solace_task.execute()
